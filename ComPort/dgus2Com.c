...
static u8 recBuffer[DGUSII_BUFF_LEN+32]; //加32为了在2级FIFO满时,清除硬件fifo剩余的数据
...
u8 DGUSII_Com_read(u8 *data, int *num)
{
    ...
    while(byteCnt) //fifo has data
    {
        ...
        if(++recOutPrt>DGUSII_BUFF_LEN-1) //缓存位置, 到缓存尾部, 回头
            recOutPrt = 0;
        ...
        if (step == 3)
        {
            if (++frameCnt > frameLength) //检数量
            {
                if (_check_sum) //开启校验
                {
                    crc16 = usMBCRC16(data, frameLength - 2);
    ...
    if(byteCnt<DGUSII_BUFF_LEN)
    {
        ...
        if (cnt)
        {
            for (i = cnt; i > 0; i--)
            {
                ...
                if (++recInPrt > DGUSII_BUFF_LEN - 1)
                    recInPrt = 0;
                if (++byteCnt >= DGUSII_BUFF_LEN) //2级fifo满
                {
