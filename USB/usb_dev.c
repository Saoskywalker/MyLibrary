...
static int ep_max_len[3] = {64,USB_DATA_PACKET_SIZE,USB_DATA_PACKET_SIZE};
unsigned char usb_tx_buf[USB_DATA_PACKET_SIZE];
int usb_tx_buf_len = 0;
int usb_tx_pos = 0;
unsigned char usb_rx_buf[USB_DATA_PACKET_SIZE];
		...
		case EP0_IDLE:
			{
				...
				if(len == 8)
				{
					...
					if((current_usb_type == USB_TYPE_USB_HID)||(current_usb_type == USB_TYPE_USB_COM)||(current_usb_type == USB_TYPE_USB_UVC))
					{								
...
void usb_handle_epn_out(int ep)
{
	...
	if (USBC_Dev_IsReadDataReady(USBC_EP_TYPE_RX))
	{
		...
		else if(current_usb_type == USB_TYPE_USB_UVC)
		{			
			// usb_uvc_out_ep_callback(usb_rx_buf,usb_rx_buf_len);
		}	
...
type com hid
*/
char usb_com_open = 0; //标记开启usb虚拟串口
int usb_device_init(unsigned char type)
{
	...
	if(current_usb_type != type)
	{
		...
		sunxi_udc_disable();
		/*中断实始化*/
		request_irq(IRQ_USBOTG, usb_irq_handle,0);
		/*UDC使能*/       
		sunxi_udc_enable();
		usbprint("usb init end...\r\n");
		usb_com_open = 1;
		return retval;
	}
	usbprint("usb_type err...\r\n");
	return -1;
}
