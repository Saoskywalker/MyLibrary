...
static void _DrawArc(int x0, int y0, int rx, int ry, int Angle0, int Angle1, int xMul, int yMul) {
  float afx[4];
  float afy[4];
	float ri = rx-(PenSize+1.5)/2;
	float ro = rx+(PenSize+1.5)/2;
  float fAngle0 = Angle0*3.1415926/180;
  float fAngle1 = Angle1*3.1415926/180;
  float sin0 = Arc_sin(fAngle0); 
  float sin1 = Arc_sin(fAngle1); 
  float cos0 = Arc_cos(fAngle0); 
  float cos1 = Arc_cos(fAngle1); 
  u32   ri2 = ri*ri;
  u32   ro2 = ro*ro;
	int y, yMax, yMin;
	afy[0] = ri*sin0;
  ...
  int xMinDisp, xMaxDisp, xMin=0,xMax=0;
    for (y=yMax; y>=yMin; y--) {
      ...
      if (xMul>0)
        LCD_HLine(xMinDisp+x0, yMul*y+y0, xMaxDisp+x0, POINT_COLOR);
      else
        LCD_HLine(-xMaxDisp+x0, yMul*y+y0, -xMinDisp+x0, POINT_COLOR);
    }
	}
}
