...
static mFILE *f_jpeg = NULL;			//JPEG文件指针
...
{ 
    UINT  rb; //读取到的字节数
    mFILE *dev=(mFILE*)jd->device;  //待解码的文件的信息，使用FATFS中的FIL结构类型进行定义
    if(buf)     				//读取数据有效，开始读取数据
    { 
        rb = MTF_read(buf,1,num,dev);//调用FATFS的f_read函数，用于把jpeg文件的数据读取出来
        return rb;        		//返回读取到的字节数目
    }else return (MTF_seek(dev,MTF_tell(dev)+num,SEEK_SET)==0)?num:0;//重新定位数据点，相当于删除之前的n字节数据 
}  
//采用填充的方式进行图片解码显示
//jd:储存待解码的对象信息的结构体
...
UINT jpeg_out_func_point(JDEC* jd,void* rgbbuf,JRECT* rect) 
{
   	...
   	for(i=0;i<height;i++)//y坐标
	{
		...
		for(j=0;j<width;j++)//x坐标
		{
			...
			pic_phy.draw_point(realx+picinfo.S_XOFF,realy+picinfo.S_YOFF,tempColor);//显示图片						
			pencolor++;
		}
	}
    return 0;    //返回0,使得解码工作继续执行 
